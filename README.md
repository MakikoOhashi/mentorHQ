...
現在はプロトタイプ段階のため、以下の制約があります。

- ログイン機能および複数学習者への対応は未実装です。
- 問題データベースは未実装で、デモ用の問題を利用しています。
- 右側の AI Coach Mind は永続保存されません。
- ページを再読み込みすると Observation は復元されますが、Thought Stream の完全な復元は行いません。
- 長期的な Learner Model は、まだ Firestore 上の全 Observation を対象としていません。
- Problem Coach Review は現在の問題のみを対象としており、Daily Review とは独立しています。
- `docs/` 配下には初期設計時のドキュメントが残っており、現行実装とは一部異なります。
- `/api/deliberate` および旧 deliberation 系モジュールは互換性維持のため残しています。

## あえて保存しないもの

MentorHQ では、AI が生成した思考そのものを資産とは考えていません。
...
MentorHQ が長期的な資産として扱うのは、
AI の思考ではなく、学習者の Observation です。

AI の解釈はモデルやプロンプトの進化とともに改善できますが、
Observation は学習者の事実として積み重なり続けます。